...
@RequestMapping : 사용자의 http 요청을 특정 메서드와 할 수 있도록 매핑하는(감싸주는) 역할을 함
	...
	RequestMethod.GET
		HTTP GET 메서드는 주로 DB에 정보를 요청하기 위해 사용됨
		사이트에서 URL를 통한 직접 요청이나 링크를 출력해야하는 상황에서 GET 메서드가 사용됨
		데이터를 서버로 전송하지는 않고, 주로 데이터를 요청하거나 조회할 때만 사용함
	RequestMethod.POST
		주로 서버로 데이터를 제출하기 위해 사용됨 
		데이터를 HTML 본문에 담아 전송하기 때문에
		GET 보다는 대량의 데이터를 처리하기에 적합 
