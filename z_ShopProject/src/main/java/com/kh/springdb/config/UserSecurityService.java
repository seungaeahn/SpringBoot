...
/*
 	데이터베이스나 외부에서 로그인하여 인증을 하기 위해서는 인증 처리를 해야 함
...
public class UserSecurityService implements UserDetailsService{
	...
	//유저에 대한 정보를 로그인할 때 userDetails 를 사용해서 로그인할 수 있는 유저가 있는지 확인할 것 
	...
	public UserDetails loadUserByUsername(String username) {
		...
		//만약에 admin user로 로그인 안된다면 로그인 분류를 role에 따라 추가로 작성
...
/*
 	...
 	getPassword() : 사용자의 비밀번호를 반환. 데이터베이스에서 암호화 처리된 형태로 저장되어 있음 
 	getUsername() : 사용자명을 반환.
 	isEnables() : 계정이 활성화돼있는지 여부를 Boolean 값으로 나타냄 
